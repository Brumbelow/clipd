//! Autostart shim.
//!
//! On Windows, `clipd install --autostart` writes
//! `HKCU\Software\Microsoft\Windows\CurrentVersion\Run\clipd` =
//! `"<exe>" --daemon` (REG_SZ). `clipd uninstall` deletes the value.
//!
//! Unsafe Win32 surface is contained in the `win` sub-module; each
//! unsafe block carries a `// SAFETY:` comment. The non-Windows stub
//! bails so calls fail loudly until a real Linux/Mac impl lands.

#[cfg(target_os = "windows")]
pub use win::{autostart_enabled, disable_autostart, enable_autostart};
...
pub fn disable_autostart() -> anyhow::Result<()> {
    // Idempotent no-op: there's nothing to remove.
    Ok(())
}

#[cfg(not(target_os = "windows"))]
pub fn autostart_enabled() -> anyhow::Result<bool> {
    Ok(false)
}
...
mod win {
    ...
    use windows::Win32::System::Registry::{
        ...
        REG_CREATE_KEY_DISPOSITION, REG_OPTION_NON_VOLATILE, REG_SAM_FLAGS, REG_SZ,
    };
    ...
    pub fn autostart_enabled() -> Result<bool> {
        ...
        let query =
            unsafe { RegQueryValueExW(hkey, PCWSTR(value.as_ptr()), None, None, None, None) };
